...
fun! netrw#Obtain(islocal,fname,...)
  ...
  if exists("b:netrw_islocal") && b:netrw_islocal
    ...
    elseif b:netrw_method == 3
      " obtain with ftp + machine, id, pk, and fname (ie. no .netrc)
      ...
      if exists("g:netrw_uid") && g:netrw_uid != ""
        if exists("g:netrw_ftp") && g:netrw_ftp == 1
          ...
          "      call Decho("filter input: ".getline('$'),'~'.expand("<slnum>"))
          if exists("s:netrw_pk") && s:netrw_pk != ""
            NetrwKeepj put ='\"'.s:netrw_pk.'\"'
          endif
        "      call Decho("filter input: ".getline('$'),'~'.expand("<slnum>"))
        elseif exists("s:netrw_pk")
          NetrwKeepj put ='user \"'.g:netrw_uid.'\" \"'.s:netrw_pk.'\"'
          "      call Decho("filter input: ".getline('$'),'~'.expand("<slnum>"))
        endif
      endif
      ...
      " -n  win32: quit being obnoxious about pk
  ...
  while ichoice <= a:0
    ...
    if exists("b:netrw_lastfile") && a:0 == 0
      ...
      if match(choice,"?") == 0
        ...
        echomsg ':Nread "machine id pk path"           uses ftp'
    ...
    elseif b:netrw_method  == 2          " read with ftp + <.netrc>
      ...
      ".........................................
    " NetRead: (ftp + machine,id,pk,filename)  NetRead Method #3 {{{3
    elseif b:netrw_method == 3           " read with ftp + machine, id, pk, and fname
    ...
    elseif     b:netrw_method  == 6
      ...
      if g:netrw_dav_cmd =~ "curl"
        ...
        if exists("g:netrw_uid") && exists("s:netrw_pk") && g:netrw_uid != ""
          NetrwKeepj put ='user '.g:netrw_uid.' '.s:netrw_pk
        endif
        NetrwKeepj put ='get '.netrw_fname.' '.tmpfile
        NetrwKeepj put ='quit'
    ...
    elseif     b:netrw_method  == 8
      ...
      if exists("g:netrw_uid") && g:netrw_uid != "" && exists("s:netrw_pk") && s:netrw_pk != ""
        call s:NetrwExe(s:netrw_silentxfer."!".g:netrw_fetch_cmd." ".s:ShellEscape(tmpfile,1)." ".s:ShellEscape(netrw_option."://".g:netrw_uid.':'.s:netrw_pk.'@'.g:netrw_machine."/".b:netrw_fname,1))
      else
  ...
  while ichoice <= a:0
    ...
    if exists("b:netrw_lastfile") && a:0 == 0
      ...
      if match(choice,"?") == 0
        ...
        echomsg ':Nwrite "machine id pk path"          uses ftp'
    ...
    elseif b:netrw_method == 2
      ...
      ".........................................
    " NetWrite: (ftp + machine, id, pk, filename) NetWrite Method #3 {{{3
    elseif b:netrw_method == 3
      ...
      if exists("g:netrw_uid") && g:netrw_uid != ""
        ...
        "      call Decho("filter input: ".getline('.'),'~'.expand("<slnum>"))
        elseif exists("s:netrw_pk") && s:netrw_pk != ""
          NetrwKeepj put ='user \"'.g:netrw_uid.'\" \"'.s:netrw_pk.'\"'
          "      call Decho("filter input: ".getline('.'),'~'.expand("<slnum>"))
        endif
      endif
      ...
      "    call Decho("filter input: ".getline('.'),'~'.expand("<slnum>"))
      " save choice/id/pk for future use
      let b:netrw_lastfile = choice
...
"                  2: ftp + <.netrc>
"                  3: ftp + machine, id, pk, and [path]filename
"                  4: scp
"                  5: http[s] (wget)
"                  6: dav
"                  7: rsync
...
fun! s:NetrwMethod(choice)
  ...
  " Patterns:
  " mipf     : a:machine a:id pk filename      Use ftp
  " mf      : a:machine filename                     Use ftp + <.netrc> or g:netrw_uid s:netrw_pk
  " ftpurm   : ftp://[user@]host[[#:]port]/filename  Use ftp + <.netrc> or g:netrw_uid s:netrw_pk
  ...
  elseif match(a:choice,ftpurm) == 0
    ...
    if curmachine != g:netrw_machine
      ...
      elseif exists("s:netrw_pk")
        " if there's a change in hostname, require pk re-entry
        unlet s:netrw_pk
      endif
    ...
    if exists("g:netrw_uid") && exists("s:netrw_pk")
      ...
      elseif s:FileReadable(expand("$HOME/.netrc")) && !g:netrw_ignorenetrc
        ...
        elseif !exists("s:netrw_pk") || s:netrw_pk == ""
          ...
          " else just use current g:netrw_uid and s:netrw_pk
        endif
        let b:netrw_method= 3
      endif
    endif
  ...
  " Method#3: Issue an ftp : "machine id pk [path/]filename" {{{3
  elseif match(a:choice,mipf) == 0
    "   call Decho("(ftp) host id pass file",'~'.expand("<slnum>"))
    let b:netrw_method  = 3
    let g:netrw_machine = substitute(a:choice,mipf,'\1',"")
    let g:netrw_uid     = substitute(a:choice,mipf,'\2',"")
    let s:netrw_pk  = substitute(a:choice,mipf,'\3',"")
    let b:netrw_fname   = substitute(a:choice,mipf,'\4',"")
    call NetUserPass(g:netrw_machine,g:netrw_uid,s:netrw_pk)
  ...
  "  endif                                        "Decho
  "  if exists("s:netrw_pk")          "Decho
  "   call Decho("s:netrw_pk <".s:netrw_pk.">",'~'.expand("<slnum>"))
  "  endif                                        "Decho
...
fun! NetUserPass(...)
  ...
  if a:0 == 0
    ...
    " change host and username if not previously entered; get new pk
    if !exists("g:netrw_machine")
    ...
    " get pk via prompting
    let s:netrw_pk= inputsecret("Enter pk: ")
    ...
    let s:netrw_hup[host].pk = s:netrw_pk
  ...
  elseif a:0 == 1
    ...
    if a:1 =~ '^ftp:'
      ...
      " access userid and pk from hup (host-user-pk) dictionary
      ...
      if exists("s:netrw_hup[host]")
        ...
        let s:netrw_pk = s:netrw_hup[host].pk
      "    call Decho("get s:netrw_hup[".host."].uid   <".s:netrw_hup[host].uid.">",'~'.expand("<slnum>"))
      "    call Decho("get s:netrw_hup[".host."].pk<".s:netrw_hup[host].pk.">",'~'.expand("<slnum>"))
      else
      ...
      if exists("g:netrw_pk")
        " ask for pk if one not previously entered
        let s:netrw_pk= g:netrw_pk
      else
        let s:netrw_pk = inputsecret("Enter pk: ")
      endif
    endif
  ...
  elseif a:0 == 3
    " enter hostname, user-id, and pk into the hup dictionary
    ...
    let s:netrw_hup[host].pk = a:3
    let g:netrw_uid              = s:netrw_hup[host].uid
    let s:netrw_pk           = s:netrw_hup[host].pk
    "  call Decho("set s:netrw_hup[".host."].uid   <".s:netrw_hup[host].uid.">",'~'.expand("<slnum>"))
    "  call Decho("set s:netrw_hup[".host."].pk<".s:netrw_hup[host].pk.">",'~'.expand("<slnum>"))
  endif

  " call Dret("NetUserPass : uid<".g:netrw_uid."> pk<".s:netrw_pk.">")
endfun
...
fun! s:NetrwMakeDir(usrhost)
  ...
  elseif b:netrw_method == 3
    " Remote mkdir: using ftp + machine, id, pk, and fname (ie. no .netrc)
    let svpos= winsaveview()
    if exists("b:netrw_fname")
      let remotepath= b:netrw_fname
    else
...
fun! s:NetrwUpload(fname,tgt,...)
  ...
  elseif type(a:fname) == 3
    ...
    elseif a:tgt =~ '^ftp:'
      ...
      elseif b:netrw_method == 3
        " upload with ftp + machine, id, pk, and fname (ie. no .netrc)
        let netrw_fname= b:netrw_fname
        ...
        if exists("g:netrw_uid") && g:netrw_uid != ""
          if exists("g:netrw_ftp") && g:netrw_ftp == 1
            ...
            "       call Decho("filter input: ".getline('$'),'~'.expand("<slnum>"))
            if exists("s:netrw_pk")
              NetrwKeepj call setline(line("$")+1,'"'.s:netrw_pk.'"')
            endif
          "       call Decho("filter input: ".getline('$'),'~'.expand("<slnum>"))
          elseif exists("s:netrw_pk")
            NetrwKeepj put ='user \"'.g:netrw_uid.'\" \"'.s:netrw_pk.'\"'
...
fun! s:NetrwRemoteFtpCmd(path,listcmd)
  ...
  elseif w:netrw_method == 3    " {{{3
    " ftp + machine,id,pk,filename:  Method #3
    setl ff=unix
    if exists("g:netrw_port") && g:netrw_port != ""
      NetrwKeepj put ='open '.g:netrw_machine.' '.g:netrw_port
    else
